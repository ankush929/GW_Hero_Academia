...
switch (state) {
	...
	case HIT:														// Apart from animation, we create the enemy hitbox in an specific frame
		image_speed = global.animFPS*hitstuned/4;
		sprite_index = sEnemyHit;
		if image_index >= 2 and !hit
		{
			hit = true;
			punch = instance_create_depth(x + sprite_get_width(sHitBoxLarger)/2*facing, y + z - sprite_height/2 - 5, depth + 1, oEnemyHitBoxLargerDmg);		// Create the punch Hit box
			punch.parentId = id;
		}
